...
def perform_EDA(
    ...
    for label, df in dfs.items():
        nrows, ncols = df.shape
        results.EDA_ready_datasets.append(DfMetadata(type=label, rows=nrows, cols=ncols))
...
def get_parser(add_help: bool = False) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="EDA Script for ML pipeline.", add_help=add_help)
