...
def build_transformer(
    ...
    if heavy_cols:
        ...
        logger.log_result(
            f"Using heavy-tail features ({len(heavy_cols)}): {heavy_cols}"
        )
    ...
    if sparse_cols:
        ...
        logger.log_result(
            f"Using sparse token features ({len(sparse_cols)}): {sparse_cols}"
        )
    ...
    if not transformers:
        raise ValueError(
            "No valid transformers could be constructed from available columns."
        )
...
def log_dropped_features(transformer, numeric_features_list, logger: MyLogger):
    ...
    if len(dropped_features) > 0:
        logger.log_result(
            f"VarianceThreshold dropped {len(dropped_features)} features: {dropped_features.tolist()}"
        )
    else:
