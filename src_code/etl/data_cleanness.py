...
def show_rows(df: pd.DataFrame, mask: pd.Series, title: str, cols: list[str] | None = None, n: int = 10):
    bad = df.loc[mask]
    print(f"\n[{title}] Count: {len(bad)}")
    if len(bad) == 0:
        print("No rows found.")
        return
    if cols is None:
        cols = [c for c in df.columns if c in {"repo", "commit", "label", "has_bug", "message", "loc_added", "loc_deleted", "files_changed", "hunks_count"}]
...
show_rows(input_df, mask_commit_bad, "Suspicious commit hash format", ["repo", "commit"])
...
mask_has_bug_not_bool = ~input_df["has_bug"].map(lambda x: isinstance(x, (bool, np.bool_)))
mask_label_bug_mismatch = input_df["label"].astype(int) != input_df["has_bug"].astype(int)

show_rows(input_df, mask_label_not_binary, "Non-binary label", ["repo", "commit", "label"])
show_rows(input_df, mask_has_bug_not_bool, "has_bug not boolean", ["repo", "commit", "has_bug"])
show_rows(input_df, mask_label_bug_mismatch, "Mismatch between label and has_bug", ["repo", "commit", "label", "has_bug"])
...
print(f"Rows with label=0 but actual buggy lines present: {mask_label0_with_bug_lines.sum()}")
...
mask_kw_bug_inconsistency = (input_df["has_bug_kw"] > 0) & ~input_df["message"].str.lower().str.contains("bug", na=False)
mask_kw_fix_inconsistency = (input_df["has_fix_kw"] > 0) & ~input_df["message"].str.lower().str.contains("fix", na=False)
...
show_rows(input_df, mask_code_embed_invalid, "Invalid code embeddings", ["repo", "commit", "code_embed"])
show_rows(input_df, mask_msg_embed_invalid, "Invalid message embeddings", ["repo", "commit", "msg_embed"])
...
corr_cols = ["loc_added", "loc_deleted", "files_changed", "hunks_count", "ast_delta", "max_func_change", "complexity_delta", "author_exp_pre"]
print(input_df[corr_cols].corr(numeric_only=True).round(3).to_string())

print("\nMean numeric features by label:")
print(input_df.groupby("label")[corr_cols + ["msg_len", "author_exp_pre", "recent_churn"]].mean().round(3).to_string())
...
summary = {
    ...
    "nonzero_files_but_zero_all_change_metrics": int(mask_nonzero_files_but_zero_everything.sum()),
...
print(input_df[['recent_churn', 'repo', 'commit', 'lines', "hunks_count"]].sort_values("recent_churn", ascending=False).head())