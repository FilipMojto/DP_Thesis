...
import tqdm
from notebooks.logging_config import MyLogger
from src_code.config import CACHE_DIR, ETL_PATH_MAPPINGS, INTERIM_DATA_DIR, LOG_DIR
...
CACHED_MESSAGES = CACHE_DIR / "commit_messages.jsonl"
...
if __name__ == "__main__":
    ...
    script_versioner = VersionedFileManager(
        file_path=INTERIM_DATA_DIR
        / "train_extended.feather"
    )
    ...
    df = parallel_fetch_messages(df=df, logger=logger, cache_path=CACHED_MESSAGES)
    save_df(df=df, df_file_path=script_versioner.next_base_output, logger=logger)
